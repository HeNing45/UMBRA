...
// rv32i_ooo_decode -- the OoO front-end decoder.
//
// Wraps rtl_p/rv32i_pipe_decode.sv and maps its pipe_ctrl_t onto the OoO-facing
// packet: operation/FU classes, operand selectors, and trap/CSR metadata.
// Instruction classification is shared with the pipeline; this wrapper adapts
// the control representation for renamed execution.

module rv32i_ooo_decode
  import fyp_cpu_pkg::*;
  import rv32i_pipeline_pkg::*;
  import rv32i_ooo_pkg::arch_reg_t;
  import rv32i_ooo_pkg::ooo_op_class_e;
  import rv32i_ooo_pkg::ooo_fu_class_e;
  import rv32i_ooo_pkg::ooo_src_sel_e;
  import rv32i_ooo_pkg::OOO_OP_ALU;
  import rv32i_ooo_pkg::OOO_OP_BRANCH;
  import rv32i_ooo_pkg::OOO_OP_JUMP;
  import rv32i_ooo_pkg::OOO_FU_ALU;
  import rv32i_ooo_pkg::OOO_FU_MULDIV;
  import rv32i_ooo_pkg::OOO_FU_LSU;
  import rv32i_ooo_pkg::OOO_SRC_REG;
  import rv32i_ooo_pkg::OOO_SRC_IMM;
  import rv32i_ooo_pkg::OOO_SRC_PC;
  import rv32i_ooo_pkg::OOO_SRC_ZERO;
  import rv32i_ooo_pkg::csr_addr_t;
  import rv32i_ooo_pkg::csr_zimm_t;
(
  ...
  output logic           is_csr,
  output csr_op_e        csr_op,        // serialized CSR operation
  output csr_addr_t      csr_addr,
  output csr_zimm_t      csr_zimm,
  output logic           is_mem,        // load/store classification
  ...
  // Shared pipeline instruction decoder.
  pipe_ctrl_t ctrl;
  ...
  // operand source selects (from the pipeline's alu_a_sel / alu_b_sel)
  assign src1_sel  = ooo_src_sel_e'(
      (ctrl.alu_a_sel == ALU_A_PC)   ? OOO_SRC_PC   :
      (ctrl.alu_a_sel == ALU_A_ZERO) ? OOO_SRC_ZERO :
                                       OOO_SRC_REG);   // ALU_A_RS1
  assign src2_sel  = ooo_src_sel_e'(
      (ctrl.alu_b_sel == ALU_B_IMM)  ? OOO_SRC_IMM  : OOO_SRC_REG);  // ALU_B_RS2
