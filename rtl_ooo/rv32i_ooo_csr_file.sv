...
// Machine-mode CSR storage and precise trap state for the scalar OoO core.
//
// CSR state changes two ways:
//   1. Explicit CSR instruction (csrrw/s/c + immediates) -> the we/waddr/wdata
//      port. The read-modify-write is computed by the core, so this port just
//      commits the final value, atomically at retirement.
...
(
  ...
  // ---- trap entry (precise, accepted at the ROB head) ----
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // These are mutually exclusive by construction (the retiring instruction is at
