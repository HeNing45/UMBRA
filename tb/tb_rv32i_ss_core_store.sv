...
// tb_rv32i_ss_core_store — store commit path (frontend + core + dmem
// model, program-driven through the REAL decoder).
//
// Proves the invariant — a store's memory write happens at COMMIT
// and only at commit — by making its violation consequential three ways:
//
...
module tb_rv32i_ss_core_store;
  ...
  int    we_outside_commit;   // dmem_we without any commit = invariant break
  always @(posedge clk) begin
    if (rst_n) begin
      ...
      if (dmem_we) begin
        ...
        if (!(|commit_fire)) we_outside_commit <= we_outside_commit + 1;
      end
  ...
  initial begin
    ...
    reset_dut();
    for (i = 0; i < 6000 && !seen_tail; i++) @(posedge clk);
    repeat (4) @(posedge clk);
    ...
    chk(we_outside_commit == 0,           "INVARIANT: dmem_we only under commit_fire", we_outside_commit[31:0], 32'h0);
    ...
    if (fails == 0)
      $display("tb_rv32i_ss_core_store PASS checks=%0d", checks);
    else
      $display("tb_rv32i_ss_core_store FAIL fails=%0d checks=%0d", fails, checks);
    $finish;
  end

  initial begin
    #600000;
    $display("tb_rv32i_ss_core_store FAIL watchdog");
    $finish;
  end
