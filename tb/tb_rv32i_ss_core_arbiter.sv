...
// completion-arbiter TB for rv32i_ss_core.
//
// Forces a real simultaneous-completion collision: an older muldiv reaches
// S_DONE on the same issue edge a younger ALU fills an *empty* ALU0 holder.
// Occupancy-only fu_ready still allows that issue (the holder is free). The
// two CDB lanes must carry both completions in ROB-age order without dropping
// or duplicating either writeback. Also checks that the LQ client
// participates in the same ROB-age ordering.

module tb_rv32i_ss_core_arbiter;
  import rv32i_ss_pkg::*;
  ...
  initial begin
    ...
    $fatal(1, "WATCHDOG: tb_rv32i_ss_core_arbiter exceeded 5000 cycles");
  end
  ...
  task automatic wait_muldiv_last_cycle();
    int wait_count;
    wait_count = 0;
    while (!(dut.u_muldiv.busy &&
             !dut.muldiv_complete.valid &&
             (dut.u_muldiv.cnt_q == 6'd0))) begin
      @(negedge clk);
      wait_count++;
      if (wait_count > 200) begin
        $fatal(1, "muldiv did not reach final pre-completion cycle");
      end
    end
  endtask
  ...
  rob_idx_t  mul_idx;
  rob_idx_t  alu_idx;
  rob_idx_t  arb_head_idx;
  completion_packet_t arb_lq_packet;
  completion_packet_t arb_other_packet;
  phys_reg_t x3_p;
  phys_reg_t x4_p;

  initial begin
    $display("[tb_rv32i_ss_core_arbiter] starting");

    reset_dut();

    // Establish ready architectural inputs: x1=6, x2=7.
    disp(32'h0000_4000, areg(0), areg(0), areg(1), OOO_FU_ALU,
         fyp_cpu_pkg::ALU_ADD, rv32i_pipeline_pkg::MD_NONE,
         OOO_SRC_REG, OOO_SRC_IMM, 32'd6);
    disp(32'h0000_4004, areg(0), areg(0), areg(2), OOO_FU_ALU,
         fyp_cpu_pkg::ALU_ADD, rv32i_pipeline_pkg::MD_NONE,
    ...
    // Older ROB entry: long muldiv. REM, not MUL: since the pipelined
    // multiplier landed, only the DIV family still runs the 32-cycle
    // count-down this scenario's wait_muldiv_last_cycle() choreography
    // needs (busy && cnt_q==0 is unreachable on the 2-stage MUL path).
    disp(32'h0000_4008, areg(1), areg(2), areg(3), OOO_FU_MULDIV,
         fyp_cpu_pkg::ALU_ADD, rv32i_pipeline_pkg::MD_REM,
         OOO_SRC_REG, OOO_SRC_REG, '0);
    mul_idx = rob_idx_t'(`ROB.tail_q - 1'b1);

    // Younger ALU waits in the IQ on p5 until the muldiv is one edge from DONE.
    force `PRF.ready_q[5] = 1'b0;
    disp(32'h0000_400c, areg(5), areg(0), areg(4), OOO_FU_ALU,
         fyp_cpu_pkg::ALU_ADD, rv32i_pipeline_pkg::MD_NONE,
         OOO_SRC_REG, OOO_SRC_REG, '0);
    alu_idx = rob_idx_t'(`ROB.tail_q - 1'b1);
    clear_inputs();

    check_bit("older muldiv index differs from younger ALU", mul_idx != alu_idx, 1'b1);

    wait_muldiv_last_cycle();
    force `PRF.ready_q[5] = 1'b1;

    // Full-picker boundary: select, operand capture, then execution.
    // Muldiv reaches S_DONE on the select edge, so wait
    // until the ALU result is registered and then check CDB age order.
    @(posedge clk);
    @(posedge clk);
    @(posedge clk);
    #1;
    check_bit("ALU completion present after exec", dut.alu0_complete.valid, 1'b1);
    ...
    check_word("muldiv result committed", `PRF.regs_q[x3_p], 32'd6);  // 6 rem 7
    check_word("ALU result committed", `PRF.regs_q[x4_p], 32'd0);

    while (dut.alu0_res_q[0].valid || dut.alu0_res_q[1].valid ||
           dut.alu1_res_q[0].valid || dut.alu1_res_q[1].valid ||
           dut.alu0_complete.valid || dut.alu1_complete.valid ||
           dut.muldiv_complete.valid || dut.agen_complete.valid ||
           dut.sq_complete.valid || dut.lq_complete.valid)
      @(posedge clk);

    // reachability delta: an LQ completion may be held concurrently with
    // any of the three existing completion clients. The older packet must win;
    // otherwise a stream of younger FU completions can starve the ROB-head load.
    arb_head_idx = dut.rob_head_idx;
    arb_lq_packet = '0;
    arb_lq_packet.valid = 1'b1;
    arb_lq_packet.rob_idx = arb_head_idx;
    arb_other_packet = '0;
    arb_other_packet.valid = 1'b1;
    arb_other_packet.rob_idx = rob_idx_t'(arb_head_idx + 1'b1);

    @(negedge clk);
    force dut.u_lsq.lq_complete = arb_lq_packet;
    force dut.alu0_complete = arb_other_packet;
    #1;
    check_bit("older LQ occupies lane 0", dut.cdb_lane_select[0][4], 1'b1);
    check_bit("younger ALU occupies lane 1", dut.cdb_lane_select[1][0], 1'b1);
    release dut.u_lsq.lq_complete;
    release dut.alu0_complete;

    // The LQ is not a fixed-priority winner: an older existing client still
    // wins, which pins the policy as age arbitration rather than LQ priority.
    arb_lq_packet.rob_idx = rob_idx_t'(arb_head_idx + 1'b1);
    arb_other_packet.rob_idx = arb_head_idx;
    @(negedge clk);
    force dut.u_lsq.lq_complete = arb_lq_packet;
    force dut.alu0_complete = arb_other_packet;
    #1;
    check_bit("older ALU occupies lane 0", dut.cdb_lane_select[0][0], 1'b1);
    check_bit("younger LQ occupies lane 1", dut.cdb_lane_select[1][4], 1'b1);
    release dut.u_lsq.lq_complete;
    release dut.alu0_complete;

    if (errors == 0) begin
      $display("[tb_rv32i_ss_core_arbiter] PASS checks=%0d", checks);
      $finish;
    end else begin
      $fatal(1, "[tb_rv32i_ss_core_arbiter] FAIL errors=%0d checks=%0d", errors, checks);
    end
  end
