...
// directed muldiv integration TB for rv32i_ss_core.
//
// Injects decoded packets directly into the core, including real
// OOO_FU_MULDIV entries. This checks the muldiv path:
//   issue -> muldiv_start -> busy FU -> completion packet -> CDB
//   -> ROB/PRF writeback -> in-order commit.

module tb_rv32i_ss_core_muldiv;
  import rv32i_ss_pkg::*;
  ...
  int errors = 0;
  int checks = 0;
  int muldiv_start_count = 0;
  int muldiv_complete_count = 0;
  bit muldiv_busy_observed = 1'b0;
  ...
  initial begin
    ...
    $fatal(1, "WATCHDOG: tb_rv32i_ss_core_muldiv exceeded 5000 cycles");
  end
  ...
  always @(posedge clk) begin
    if (rst_n) begin
      ...
      if (dut.muldiv_complete.valid) begin
        muldiv_complete_count++;
      end
      if (dut.u_muldiv.busy && !dut.muldiv_complete.valid) begin
        muldiv_busy_observed = 1'b1;
      end
      // Occupancy-only admission: a live completion holder is not start
      // capacity. The second/third ops must start after the prior holder
      // has drained, not on the grant cycle.
      if (dut.u_muldiv.start_accept && !dut.muldiv_complete.valid &&
          (muldiv_complete_count > 0)) begin
        muldiv_refill_count++;
      end
    end
  end
  ...
  task automatic disp(
    ...
    decoded_valid    = 1'b1;
    decoded_pc       = pc;
    decoded_instr    = pc;  // only used as a commit-trace token in this TB
    decoded_rs1      = rs1;
    decoded_rs2      = rs2;
    decoded_rd       = rd;
    decoded_rd_we    = 1'b1;
    ...
    while (decoded_ready !== 1'b1) begin
      ...
      if (wait_cycles > 200) begin
        $fatal(1, "dispatch stuck: pc=%08h ROB.count=%0d iq_alloc_ready=%0b muldiv_busy=%0b",
               pc, `ROB.count_q, dut.iq_alloc_ready, dut.muldiv_busy);
      end
    end
    @(posedge clk);
    @(negedge clk);
  ...
  initial begin
    $display("[tb_rv32i_ss_core_muldiv] starting");

    reset_dut();

    // x1 = 6, x2 = 7
    disp(32'h0000_2000, areg(0), areg(0), areg(1), OOO_FU_ALU,
         fyp_cpu_pkg::ALU_ADD, rv32i_pipeline_pkg::MD_NONE,
         OOO_SRC_REG, OOO_SRC_IMM, 32'd6);
    disp(32'h0000_2004, areg(0), areg(0), areg(2), OOO_FU_ALU,
         fyp_cpu_pkg::ALU_ADD, rv32i_pipeline_pkg::MD_NONE,
         OOO_SRC_REG, OOO_SRC_IMM, 32'd7);

    // x3 = 42, x4 = 6, x5 = 0. x4/x5 depend on the mul result.
    disp(32'h0000_2008, areg(1), areg(2), areg(3), OOO_FU_MULDIV,
         fyp_cpu_pkg::ALU_ADD, rv32i_pipeline_pkg::MD_MUL,
         OOO_SRC_REG, OOO_SRC_REG, '0);
    disp(32'h0000_200c, areg(3), areg(2), areg(4), OOO_FU_MULDIV,
         fyp_cpu_pkg::ALU_ADD, rv32i_pipeline_pkg::MD_DIV,
         OOO_SRC_REG, OOO_SRC_REG, '0);
    disp(32'h0000_2010, areg(3), areg(2), areg(5), OOO_FU_MULDIV,
    ...
    while (`ROB.count_q !== '0) begin
      @(posedge clk);
      drain++;
      if (drain > 1000) $fatal(1, "drain stuck: ROB.count=%0d", `ROB.count_q);
    end
    ...
    check_word("commit_order==5", word_t'(`ROB.commit_order_q), 32'd5);
    check_word("x3 mul", `PRF.regs_q[x3_p], 32'd42);
    check_word("x4 div", `PRF.regs_q[x4_p], 32'd6);
    check_word("x5 rem", `PRF.regs_q[x5_p], 32'd0);
    check_word("muldiv starts", word_t'(muldiv_start_count), 32'd3);
    check_word("muldiv completions", word_t'(muldiv_complete_count), 32'd3);
    check_bit("muldiv busy before completion", muldiv_busy_observed, 1'b1);
    check_bit("later muldiv started after a prior completion drained",
              muldiv_refill_count > 0, 1'b1);

    if (errors == 0) begin
      $display("[tb_rv32i_ss_core_muldiv] PASS checks=%0d", checks);
      $finish;
    end else begin
      $fatal(1, "[tb_rv32i_ss_core_muldiv] FAIL errors=%0d checks=%0d", errors, checks);
    end
  end
