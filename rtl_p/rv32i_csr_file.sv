...
// Machine-mode CSR storage and precise trap state for the pipeline core.
//
// CSR state changes two ways:
//   1. Explicit CSR instruction (csrrw/s/c + immediates) -> the we/addr/wdata
//      port. The read-modify-write is computed by the W-stage in the core, so
//      this port just commits the final value, atomically at retirement.
...
//
// mtvec and mepc are also exposed as direct outputs so the core's W-stage trap
// unit can form the redirect target (mtvec on entry, mepc on mret) without
// borrowing the CSR-instruction read port.
...
(
  ...
  // ---- trap entry (precise, resolved at W) ----
  ...
  // ---- trap return ----
  input  logic        mret_we,     // 1 -> an mret is retiring this cycle
  ...
  localparam int         MIE_BIT  = 3;     // machine interrupt enable ("am I accepting interrupts?")
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // These are mutually exclusive by construction (the instruction in W is at
      // most one of {trapping, mret, csr-write}); the ordering is defensive.
      if (trap_we) begin
        mepc_q  <= trap_pc;
        mcause_q <= trap_cause;
        ...
        unique case (addr)
          ...
          default: begin
            // Writes to unsupported CSR addresses leave state unchanged.
          end
        endcase
      end
    end
